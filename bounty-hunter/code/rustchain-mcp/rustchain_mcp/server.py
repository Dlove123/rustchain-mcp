...
def green_tracker() -> str:
    """Elyan Labs Green Tracker — Machines Preserved from E-Waste.

    Tracks every machine saved from e-waste, with fleet data including
    machines count, years preserved, architectures, power draw, and CO2 saved.
    """
    try:
        r = get_client().get("https://rustchain.org/preserved.html", timeout=10)
        r.raise_for_status()
        
        # Parse key stats from HTML
        html = r.text
        
        # Extract machine count
        import re
        machine_match = re.search(r'>(\d+,?\d*)<', html)
        machine_count = machine_match.group(1) if machine_match else "N/A"
        
        # Extract CO2 saved
        co2_match = re.search(r'([\d,]+\.?\d*)\s*tonnes? CO2', html, re.IGNORECASE)
        co2_saved = co2_match.group(1) if co2_match else "N/A"
        
        # Extract power draw
        power_match = re.search(r'([\d,]+\.?\d*)\s*kW', html)
        power_draw = power_match.group(1) if power_match else "N/A"
        
        return f"""# 🌱 Elyan Labs Green Tracker

## Machines Preserved from E-Waste

Every CPU deserves a voice. We rescue vintage hardware from landfills and give them new life mining RustChain.

## Fleet Stats

| Metric | Value |
|--------|-------|
| **Machines Saved** | {machine_count} |
| **CO₂ Emissions Prevented** | {co2_saved} tonnes |
| **Total Power Draw** | {power_draw} kW |

## Architectures Preserved

- PowerPC G4/G5 (2003-2006)
- IBM POWER8 (2013-2017)
- Intel Xeon (2010-2018)
- Apple Silicon (2020+)
- AMD EPYC (2017+)

## Environmental Impact

By extending the life of vintage hardware, we prevent:
- E-waste in landfills
- Manufacturing emissions from new hardware
- Rare earth mineral extraction

## Learn More

- Website: https://rustchain.org/preserved.html
- RustChain: https://rustchain.org
"""
    except Exception as e:
        return f"# 🌱 Elyan Labs Green Tracker\n\nError fetching data: {e}\n\nVisit https://rustchain.org/preserved.html for latest stats."
